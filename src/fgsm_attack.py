...
def fgsm_attack(atk_model, images_01, labels, epsilon, device="cpu"):
    images_01 = images_01.to(device)
    labels = labels.to(device)
    loss_fn = nn.CrossEntropyLoss()

    adv = images_01.clone().detach().requires_grad_(True)
    loss = loss_fn(atk_model(adv), labels)
    grad = torch.autograd.grad(loss, adv)[0]
    adv = images_01 + epsilon * grad.sign()
    adv = adv.clamp(0, 1).detach()
    return adv


def fgsm_attack_batched(
    atk_model,
    norm_layer,
    images_norm,
    labels,
    ...
    for i in range(0, len(images_norm), batch_size):
        ...
        adv_01 = fgsm_attack(atk_model, img_01, lbl_batch, epsilon, device)
...
def save_results(clean_preds, adv_preds, labels, epsilon):
    lines = []
    lines.append("=" * 45)
    lines.append("FGSM ATTACK RESULTS")
    lines.append("=" * 45)
    lines.append(f"Model:       VGG-16 (CIFAR-10)")
    lines.append(f"Attack:      FGSM")
    ...
    for i, cls in enumerate(CIFAR10_CLASSES):
        ...
        if idx.sum() == 0:
            continue
        c = (clean_preds[idx] == labels[idx]).float().mean().item() * 100
        a = (adv_preds[idx] == labels[idx]).float().mean().item() * 100
        cm = clean_preds[idx] == labels[idx]
        f = cm & (adv_preds[idx] != labels[idx])
        asr_ = f.float().sum() / cm.float().sum().clamp(min=1) * 100
        lines.append(f"{cls:<12} {c:>7.1f}% {a:>7.1f}% {asr_:>7.1f}%")

    Path("fgsm_results/results.txt").write_text("\n".join(lines))
    print("Saved fgsm_results/results.txt")
...
def visualize_grid(clean_01, adv_01, labels, n_per_class=10):
    ...
    fig.text(
        0.5,
        0.99,
        "Clean vs Adversarial by Class (FGSM)",
        ha="center",
        fontsize=11,
        fontweight="bold",
    ...
    fig.text(
        0.5,
        0.972,
        f"FGSM  |  ε=0.03  |  L∞  |  VGG-16 on CIFAR-10",
        ha="center",
        fontsize=7,
        color="#777",
        transform=fig.transFigure,
    ...
    out = Path("fgsm_results/clean_vs_adversarial_grid.png")
    plt.savefig(out, dpi=320, bbox_inches="tight")
    print("Saved fgsm_results/clean_vs_adversarial_grid.png")
    webbrowser.open(f"file://{out.resolve()}")


if __name__ == "__main__":
    WEIGHTS_PATH = "models/cnn.pt"
    FGSM_SAVE_PATH = "fgsm_results/fgsm_images.pt"
    EPSILON = 0.03
    BATCH_SIZE = 128
    N_VIS = 10

    Path("fgsm_results").mkdir(parents=True, exist_ok=True)
    ...
    if Path(FGSM_SAVE_PATH).exists():
        print("Loading saved FGSM images...")
        fgsm_01 = torch.load(FGSM_SAVE_PATH)
    else:
        print(f"\nRunning FGSM attack (ε={EPSILON})...")
        fgsm_01 = fgsm_attack_batched(
            atk_model,
            norm_layer,
            images_norm,
            labels,
        ...
        torch.save(fgsm_01, FGSM_SAVE_PATH)
        print(f"Saved FGSM images to {FGSM_SAVE_PATH}")

    fgsm_preds, _ = evaluate(atk_model, fgsm_01, labels, tag="FGSM", device=device)
    evaluate_per_class(clean_preds, fgsm_preds, labels)
    compute_asr(clean_preds, fgsm_preds, labels)
    save_results(clean_preds, fgsm_preds, labels, EPSILON)
    visualize_grid(images_01, fgsm_01, labels, n_per_class=N_VIS)
